...
| **provider_can_adopt_request** | Adopt action updates status to `ADOPTED`, funding_source to `PROVIDER_ADOPTION`. No wallet changes. |
| **provider_can_approve_request** | Approve action updates status to `PROVIDER_APPROVED`, funding_source to `CITY_FUND`. Transfers `reserved_amount` from system wallet to provider wallet. |
| **provider_cannot_approve_when_city_fund_has_insufficient_balance** | When system wallet balance (10 SAR) is less than `reserved_amount` (50 SAR), approve is blocked. Request stays `PENDING`, session has error. |
| **provider_can_reject_request_with_reason** | Reject action updates status to `PROVIDER_REJECTED` with `rejection_reason_code` and `rejection_reason_note`. |
| **provider_cannot_act_on_non_pending_request** | Actions (adopt, approve, reject) are only allowed when request status is `PENDING`. Non-pending requests return error. |
...
- **System wallet:** Ewallet with `owner_type = SYSTEM`, balance 100 SAR (for approve test)
- **Provider profile:** Creates provider ewallet via `ProviderProfile::booted()`
- **Menu item:** ProviderMenuItem (50 SAR)
- **Request:** PENDING request with `reserved_amount` 50 SAR

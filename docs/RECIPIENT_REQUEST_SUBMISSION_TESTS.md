...
| Test | Description |
|------|-------------|
| **recipient_can_submit_multi_item_request_successfully** | Happy path: recipient submits a valid multi-item request. Verifies redirection, database insertion, correct status (`PENDING`), reserved amount, and price snapshots. |
...
| **adopted_request_does_not_count_towards_allowance** | Verifies that `ADOPTED` requests (provider pays) do not count toward the limit. Recipient can submit new request after an adopted request. |
...
1. **Weekly allowance:** 400 SAR per recipient per week; only REDEEMABLE and FULFILLED requests count (see `RecipientAllowanceService`).
2. **Status exclusions:** PENDING, ADMIN_REJECTED, ADOPTED (PROVIDER_ADOPTION) do not count toward allowance.

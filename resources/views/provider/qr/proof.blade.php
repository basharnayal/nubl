<x-app-layout title="{{ __('Upload Proof') }}" is-header-blur="true">
    <div class="pt-4">
        ...
        <div class="card max-w-2xl mx-auto overflow-hidden p-0 shadow-sm sm:p-0">
            {{-- Order summary --}}
            <div
                class="border-b border-primary/15 bg-gradient-to-b from-primary/[0.06] to-transparent px-4 py-5 dark:border-accent/20 dark:from-accent/[0.08] sm:px-6">
                <div class="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        <h2 class="text-base font-bold text-slate-800 dark:text-navy-50">{{ __('Order Details') }}</h2>
                        <p class="mt-1 text-sm text-slate-600 dark:text-navy-300">
                            <span>{{ __('Request #') }}{{ $redemption->request->id }}</span>
                            <span class="mx-1.5 text-slate-300 dark:text-navy-600" aria-hidden="true">&bull;</span>
                            <span>{{ __('Reference') }}:</span>
                            <span class="font-mono text-slate-800 dark:text-navy-100" dir="ltr">{{ \App\Support\PseudonymousRequestId::make($redemption->request->id) }}</span>
                        </p>
                    </div>
                    <div
                        class="rounded-xl border border-primary/20 bg-primary/10 px-4 py-2 text-end dark:border-accent/25 dark:bg-accent/15">
                        <p class="text-xs font-medium text-slate-600 dark:text-navy-300">{{ __('Reserved Amount') }}</p>
                        <p class="text-lg font-bold tabular-nums text-primary dark:text-accent-light">
                            {{ number_format($redemption->request->reserved_amount, 2) }}
                            <span class="text-sm font-semibold">{{ __('SAR') }}</span>
                        </p>
                    </div>
                </div>
            </div>

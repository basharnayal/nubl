    ...
    <div class="w-full">
        <div class="rounded-xl border border-slate-200 bg-white p-6 sm:p-8 shadow-sm">
            <div class="flex items-start gap-4">
                <div class="flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center {{ auth()->user()?->status === 'rejected' ? 'bg-red-100' : 'bg-nubl-gold-100' }}">
                ...
                <div class="flex-1">
                    @if(auth()->user()?->status === 'rejected')
                        <h1 class="text-xl font-semibold text-slate-900 mb-2">{{ __('Your application was rejected') }}</h1>

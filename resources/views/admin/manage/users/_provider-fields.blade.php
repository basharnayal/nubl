@php
    $docs = isset($user) ? $user->providerDocuments : null;
@endphp

{{-- Personal & Business --}}
<div class="space-y-4">
    <h4 class="text-sm font-semibold uppercase tracking-wide text-slate-600 dark:text-navy-300">{{ __('Personal & Business Information') }}</h4>
    <p class="text-xs text-slate-500 dark:text-navy-400">{{ __('Phone and email are edited in Account Information above.') }}</p>
...
<div class="space-y-4 border-t border-slate-200 pt-6 dark:border-navy-500">
    ...
    @isset($user)
        <div>
            ...
            <p class="mt-0.5 text-xs text-slate-500 dark:text-navy-400">{{ __('Set open/close times or mark each day as closed.') }}</p>
            @include('admin.manage.users._provider-operating-hours', [
                'weekdayKeys' => $weekdayKeys ?? array_keys(config('provider.weekdays')),
                'oh' => $old['operating_hours'] ?? [],
            ])
        </div>
    @endisset
    <div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div>
            <x-input-label for="adoption_support" :value="__('Adopt orders as community support')" required />
...
<div class="space-y-4 border-t border-slate-200 pt-6 dark:border-navy-500">
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
        <div>
            ...
            @if(isset($user) && ($docs?->business_license_path ?? null))
                @php $ext = strtolower(pathinfo($docs->business_license_path, PATHINFO_EXTENSION)); @endphp
                <a href="{{ route('admin.users.file', [$user, 'business_license']) }}" target="_blank" class="mt-2 block">
                    @if(in_array($ext, ['jpg','jpeg','png','gif','webp']))
                        <img src="{{ route('admin.users.file', [$user, 'business_license']) }}" alt="{{ __('Business License') }}" class="max-h-64 max-w-full rounded-lg border object-contain" />
                    @else
                        <span class="inline-flex items-center rounded-lg bg-primary/10 px-3 py-2 text-sm font-medium text-primary hover:bg-primary/20 dark:bg-accent/10 dark:text-accent-light dark:hover:bg-accent/20">{{ __('Business License') }}</span>
                    @endif
                </a>
            @endif
            <input id="business_license" type="file" name="business_license" accept=".pdf,.jpg,.jpeg,.png" class="form-input form-input-lineone mt-2 w-full file:mr-4 file:rounded-lg file:border-0 file:bg-primary file:px-4 file:py-2 file:text-white file:hover:bg-primary-focus dark:file:bg-accent dark:file:hover:bg-accent-focus" {{ ($docsRequired ?? true) ? 'required' : '' }} />
            @if(!($docsRequired ?? true))
        ...
        <div>
            ...
            @if(isset($user) && ($docs?->id_or_iqama_path ?? null))
                @php $extId = strtolower(pathinfo($docs->id_or_iqama_path, PATHINFO_EXTENSION)); @endphp
                <a href="{{ route('admin.users.file', [$user, 'id_or_iqama']) }}" target="_blank" class="mt-2 block">
                    @if(in_array($extId, ['jpg','jpeg','png','gif','webp']))
                        <img src="{{ route('admin.users.file', [$user, 'id_or_iqama']) }}" alt="{{ __('ID / Iqama') }}" class="max-h-64 max-w-full rounded-lg border object-contain" />
                    @else
                        <span class="inline-flex items-center rounded-lg bg-primary/10 px-3 py-2 text-sm font-medium text-primary hover:bg-primary/20 dark:bg-accent/10 dark:text-accent-light dark:hover:bg-accent/20">{{ __('ID / Iqama') }}</span>
                    @endif
                </a>
            @endif

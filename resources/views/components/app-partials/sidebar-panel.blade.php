...
<div class="sidebar-panel">
    <div class="flex h-full grow flex-col bg-white pl-[var(--main-sidebar-width)] dark:bg-navy-750">
        ...
        <!-- Sidebar Panel Body -->
        <div class="h-[calc(100%-4.5rem)] overflow-x-hidden pb-6" x-init="typeof SimpleBar !== 'undefined' && ($el._x_simplebar = new SimpleBar($el))">
                                    ...
                                    @foreach ($menu['submenu'] as $subKey => $submenu)
                                        <li @if (($submenu['route_name'] ?? '') === $pageName) x-init="$el.scrollIntoView({block:'center'}); expanded = true" @endif>
                                            <a href="{{ Route::has($submenu['route_name'] ?? '') ? route($submenu['route_name']) : '#' }}"
                                                class="flex items-center justify-between p-2 text-xs-plus tracking-wide outline-hidden transition-[color,padding-left] duration-300 ease-in-out hover:pl-4 {{ ($submenu['route_name'] ?? '') === $pageName ? 'text-primary dark:text-accent-light font-medium' : 'text-slate-600 hover:text-slate-800 dark:text-navy-200 dark:hover:text-navy-50' }}">

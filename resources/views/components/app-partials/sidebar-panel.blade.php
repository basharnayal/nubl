...
<div class="sidebar-panel">
    <div class="flex h-full grow flex-col bg-white pl-[var(--main-sidebar-width)] rtl:pl-0 rtl:pr-[var(--main-sidebar-width)] dark:bg-navy-750">
        <!-- Sidebar Panel Header -->
        <div class="flex h-18 w-full items-center justify-between ps-4 pe-1">
        ...
        <!-- Sidebar Panel Body -->
        <div class="h-[calc(100%-4.5rem)] overflow-x-hidden pb-6" {{ app()->getLocale() === 'ar' ? 'data-simplebar-direction="rtl"' : '' }} x-init="typeof SimpleBar !== 'undefined' && ($el._x_simplebar = new SimpleBar($el))">
                        ...
                        @if (isset($menu['submenu']))
                            <li x-data="accordionItem('{{ $keyMenu }}')">
                                <a :class="expanded ? 'text-slate-800 font-semibold dark:text-navy-50' : 'text-slate-600 dark:text-navy-200'"
                                    ...
                                    class="flex items-center justify-between py-2 text-xs-plus tracking-wide outline-hidden transition-[color,padding-inline-start] duration-300 ease-in-out hover:text-slate-800 dark:hover:text-navy-50"
                                    href="javascript:void(0);">
                                    <span>{{ $menu['title'] }}</span>
                                    <svg :class="expanded && '{{ app()->getLocale() === 'ar' ? '-rotate-90' : 'rotate-90' }}'" xmlns="http://www.w3.org/2000/svg"
